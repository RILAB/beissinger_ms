...
After establishing that genic sites are not evolving neutrally, we used sites $>5$kb from genes to estimate the parameters of a simple domestication bottleneck model  (Figure \ref{fig:bottleneck}). Here, we make the assumption that non-genic DNA is less selectively constrained than genic DNA. This assumption is consistent with observations purifying selection described in a later section, but we should highlight that there are known instances of non-genic sites under selection in maize\cite{studer2011}.
...
In addition to our simple bottleneck model, and because our modest sample size of fully sequenced individuals has limited power to infer recent population expansion, we investigated two alternative approaches for demographic inference.  First, we utilized genotyping data from more than 4,000 maize landraces \cite{Hearne2015} to estimate the modern maize effective population size using on a rare variant-based estimate of  $\theta$ \cite{fu1993} (see online methods). Because of the larger population size, this sample can be used to estimate the abundance of rare alleles and is therefore informative for recent population expansion. 
