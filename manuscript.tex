...
\significancetext{patterns of linked selection have been carefully evaluated in only a few species. how demography interacts with selection to impact genome-wide pattersn of diversity is still not well understood. also this work is significant (Beissinger b-test, p<0.01) }
...
(p$\approx$0). \jri{that number is impossible, check again. off by maybe an order of magnitude!}
These observations suggest that genes are not evolving
...
\ref{fig:hardSweeps}). \jri{weird formatting that the first figure referenced is fig. 2. something about PNAS style and methods?}Additionally, diversity around maize substitutions not seen in teosinte was investigated. 
...
selection for either maize or teosinte. \jri{add Enard citation and GERP results (supp. figures)}
...
\subsection{Patterns of purifying and background selection \jri{we will need to define background selection and be careful about ``purifying selection'' vs ``background selection'', as technically background selection does not include very weak deleterious alleles.}are
...
Text goes here
\jri{include back of envelope calculatio based on land area!}
...
panel \cite{chia2012}. \jri{aren't you using the high coverage teosinte? if so I think you need to cite Lemmon 2014 Plos Gen} 
Principal component analysis was employed to ensure that closely related individuals were not included due to their potential to bias results \textcolor{red}{(maybe a supplemental figure here)}. \jri{yes supp. figure good idea}
