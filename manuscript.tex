...
\dropcap{T}he genetic properties of populations are determined by a constant interplay between genetic drift and selection. Drift is the consequence of a finite population size, such that random sampling of gametes every generation will incrementally lead to changes in genetic variabiligy \cite{dobzhansky1957}. In other words, the demographic history of a population, which includes its population size fluctuations and mating patterns, has a long-lasting impact on the patterns of DNA variability displayed by that population. Conversely, selection systematically alters a population by favoring particular alleles at the expense of others. In contrast to drift, which results from demographic history, selection is the result of the relationship between genotypes, phenoytpes, and ultimately fitness. A classical approach for studying selection is to remove the genome-wide effects of demography and assume that selection will leave locus-specific evidence \cite{Wall2002, akey2009}. Conversely, demography is often researched by excluding potentially selected sites \cite{voight2005, luikart2003, gutenkunst2009}.\jri{pretty good first paragraph. i will edit later, but i think more or less OK}

However, recent work has shown that demography and selection do not operate independently to determine genetic variability, in large part because linkage allows the effects of selection to be wide-ranging \cite{li2012,slotte2014}. \jri{need to cite older stuff. this isn't new. what about maynard smith and haigh? gillespie? lots of popgen on this}
For example, in \emph{Drosophila} the majority of the genome is impacted by the effects of selection through linkage
\cite{sella2009}. One technique that has been utilized to estimate demographic parameters independently of selective parameters is to limit analyses to only sites far from genes\cite{gazave2014}, but as \emph{Drosophila} demonstrates, it is
difficult to be certain, without species-specific knowledge, that sites even in gene-poor regions of the genome are not influenced by linked selection. Therefore, characterizing selection, demography or both, first requires an understanding of linked selection. It has already been established that patterns of linked selection are heavily influenced by population size ($N_e$) \cite{cutter2013, slotte2014}, but nearly all species including humans \cite{reich1998}, domesticates \cite{hyten2006, bovine2009}, ecological \cite{storfer2014}, and model species \cite{duchen2013} have experienced recent or ancient changes in $N_e$. Although much is known about how the long-term average $N_e$ affects linked selection, much less is understood about the immediate effects and dynamics of changing in $N_e$ on patterns of linked selection. \jri{main points are there. tempted to split into two paragraphs. 1) selection/drift not independent because of linked selection. very briefly explain it -- bgs vs purifying vs hard/soft. linked selection impacted by Ne. BGS models, hard sweep models, standing variaton (hermisson/pennings petrov/messer) etc. 2) but always assume long-term Ne. pops have gone through demography, this has potential to impact both selection and drift}
