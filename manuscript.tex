...
We removed \X teosinte and \X maize individuals that appeared as outliers in an initial principal component analysis  (Figure S5), leaving 13 teosinte and 23 maize that were used for all subsequent analyses (Table S1).

\subsection{Physical and genetic maps}
Sequences were mapped to the maize B73 version 3 reference genome \cite{schnable2009} (ftp://ftp.ensemblgenomes.org/pub/plants/release-22/fasta/zea\_mays/dna/) as described by \cite{chia2012}.\jri{did we use their bams? I thought we used bwa bams, which are not listed and weren't used for hapmap2. if that's the case, we need to mention this difference. either way, we need to include a link to the bamfiles too.}
All analyses made use of uniquely mapping reads with mapping quality score $\geq 30$ and bases with base quality score $\geq 20$, adjusting quality scores around indels. 
...
We assumed individuals were fully inbred and treated each line as a single haplotype. \jri{is doSaf -2 equivalent to using the inbreeding coefficient? in general i removed the options because they don't make sense to casual reader and the interested reader can find the script}
...
We modeled an a teosinte of constant effective population size $N_a$ individuals that at time $T_b$ generations in the past gives rise to a maize population of size $N_b$ which grows exponentially to size $N_{maize}$ in the present (Figure \ref{fig:bottleneck})
The model includes migration each generation of $M_{mt}$ individuals from maize to teosinte and $M_{tm}$ individuals from teosinte to maize.  
We fixed $N_a$ using $\delta\alpha\delta{i}$'s estimation of the population mutation rate $\theta=4N_a\mu$ and a mutation rate of $3 \times 10^{-8}$ \cite{clark2005}. 
We estimated all other parameters using 1,000 $\delta\alpha\delta{i}$ optimizations of the model using ``$\delta\alpha\delta{i}$.Inference.optimize\_log\_fmin'' optimizer \jri{is this last bit about the optimizer in your code? if so, i'd ditch} setting initial parameter values at $\frac{N_b}{N_a}=0.02$, $\frac{N_{maize}}{N_a}=3$, $\frac{T_b}{2N_a}=0.04$, and $\frac{M_{mt}}{N_c}=\frac{M_{tm}}{N_a}=10^{-5}$, where the maize population size $Nc$ at time $i$ generations in the past is $N_c=N_ae^{\frac{ln(\frac{N_{maize}}{N_a})(T_b-i)}{T_b}}$.
...
We allowed initial values to be randomly purterbed by a factor of 2, and set upper and lower bounds such that $10^{-7} \eq \frac{N_b}{N_a} \leq 2$, $10^{-7} \leq \frac{N_{maize}}{N_a} \leq{200}$, $0 \leq \frac{T_b}{2N_a} \leq 1$, $10^{-10}\leq \frac{M_{tm}}{N_a}\leq 10^{-3}$, and $10^{-10}\leq \frac{M_{mt}}{N_c}\leq 10^{-3}$
We report parameter estimates from the optimization run with the highest log-likelihood.

We additionally made use of the large genotyping data set of more than 4,000 maize landraces \citex to calculate a singleton-base estimate of the modern maize effective population size.   
