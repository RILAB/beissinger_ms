...
\author{Timothy M. Beissinger\affil{1}{US Department of Agriculture, Agricultural Research Service, Columbia, MO, USA}\affil{2}{Division of Plant Sciences, University of Missouri, Columbia, MO, USA}\affil{3}{Dept. of Plant Sciences, University of California, Davis, CA, USA}, Li Wang\affil{4}{Department of Ecology, Evolution, and Organismal Biology, Iowa State University, Ames, IA, USA}\affil{5}{Genome Informatics Facility, Iowa State University, Ames, IA, USA}, Kate Crosby\affil{1}{}, Arun Durvasula\affil{1}{}, Matthew B. Hufford\affil{4}{}, \and Jeffrey Ross-Ibarra\affil{1}{}\affil{6}{Genome Center and Center for Population Biology, University of
...
\begin{abstract}
  The genetic variability present in a population results from a constant interaction between drift and natural selection. 
To quantify the interplay between demography (which drives drift) and selection, we investigated genetic diversity in maize, one of the most important crops for food, feed, and fuel world-wide. 
Our sample included whole genome sequence data from 23 maize and 13 teosinte individuals. 
We obtained a complete estimate of the population size fluctuations and other demographic parameters experienced by maize as it was domesticated from teosinte. 
Here, we show that maize went through a domestication bottleneck with a population size of approximately 5\% that of teosinte before it experienced rapid population size expansion post-domestication. 
We observe that hard sweeps on genic mutations are not the primary force driving maize evolution. 
We find that a reduced population size during domestication decreased the efficiency of purifying selection to purge deleterious alleles from maize. 
However, expansion after domestication has since increased the efficiency of purifying selection to levels exceeding those seen in teosinte. 
Our results demonstrate that demographic and selective history in the ancient and recent past both contribute to the levels of genetic variability that are present today. 
Our results also show that for bottlenecked species, rapid post-bottleneck expansion can have wide-ranging impacts on diversity that conflict with the expected loss of diversity that occurs during the bottleneck itself. %JRI NOT DONE
...
%\jri{careful here with these citations. we're effectively calling these people out, so make sure they're really doing what you say they are!}
%TB says: not so! Akey's is a review where he calls others out. (got rid of the other)
...
Although much is known about how the long-term average $N_e$ affects linked selection \jri{citations needed}, relatively little is understood about the immediate effects of more recent changes in $N_e$ on patterns of linked selection.
...
In this study, we leverage the maize-teosinte study system to study these effects by first estimating the parameters of the maize domestication bottleneck using whole-genome resequencing data and then investigating the relative importance of different forms of linked selection on diversity in the recent and ancient past. 
...
To investigate how demography and linked selection have shaped patterns of diversity in maize and teosinte, we analyzed data from 23 maize and 13 teosinte genomes from the maize HapMap 2 and HapMap 3 projects \cite{chia2012, hapmap3}. As a preliminary step, we evaluated levels of diversity inside and outside of genes across the genome. We find broad differences in genic and intergenic diversity consistent with earlier results  \cite{hufford2012}(Figure \ref{fig:diversity}).  In maize, mean pairwise diversity ($\pi$) within genes was significantly lower than at sites at least 5 kb away from genes (0.00668 vs 0.00691, $p<2\times 10^{-44}$). 
...
\end{center}
\caption{Pairwise diversity $\pi$ (A,B) and Tajimas D (C,D) in 1kb windows from genic and nongenic regions of maize (A,C) and teosinte (B,D). Shown in A and B are means $\pm$ one standard deviation.   \label{fig:diversity} }
\end{figure}
...
Finally, while direct comparison of pairwise and singleton diversity within taxa is consistent with   non-equilibrium dynamics in teosinte as indicated in our coalescent-based demographic analysis, these too reveal much stronger differences in maize (Figure \ref{sFig:singletonPi}). \jri{tempted to remove phrase "as indicated in our coalescent-based demographic analysis"}
...
\subsection{Hard sweeps do not shape genome-wide diversity in maize} %JRI NOT DONE
...
The absence of evidence for a genome-wide impact of hard sweeps in coding regions differs markedly from observations in \emph{Drosophila} \cite{sattath2011} and \emph{Capsella} \cite{williamson2014}, but is consistent with data from humans \cite{hernandez2011,pritchard2010genetics}, but is consistent with estimates of the percentage of nonsynonmyous substitutions fixed by natural selection \citep{ross2009,sella2009,eyre2009estimating,williamson2014}. 
While larger long-term $N_e$ likely explains some of this difference, we see little change in the importance of hard sweeps in genes even looking at singleton diversity in modern maize \jri{reference appropriate figure}, suggesting other factors may contribute to these differences.  
If mutational target size scales with genome size, the larger genomes of human and maize may offer more possibilities for noncoding loci to contribute to adaptation, with hard sweeps on nonsynonymous variants then playing a relatively small role 
In line  with this argument, studies of local adaptation in taxa with large genomes such as teosinte \citep{pyhajarvi2013} and humans \citep{fraser2013gene} find enrichment for loci in regulatory regions, while similar analyses in the small \emph{Arabidopsis} genome finds an enrichment for nonsynonymous variants.

%\jri{what \% of the windows of top H12 score genome-wide occur outside of genes? is that a number to include here?}
% TB Says: No, because windows were 200 SNPs so even windows that don't correspond to a gene could include a gene.

%\mbh{additional consideration of non-genic regions in both the results and discussion would probably be a good idea given Tanja and Sho's results}
% TB Says: I agree, but I don't think we have the data to really get at the relative contribution of non-genic regions.


\subsection{Demography influences the efficiency of purifying selection} %JRI NOT DONE
One of our more striking findings is that the impact of purifying selection on maize and teosinte variability has varied through time. Although we observe a more pronounced decrease in $\pi$ around genes in teosinte than maize (Figure \ref{fig:purify}A), we see a greater decrease in singleton diversity in maize than teosinte (Figure \ref{fig:purify}B). Purifying selection is more efficient in larger populations  \cite{kimura1984}, and our observations using $\pi$  are thus consistent with our demographic model and previous work \cite{eyre1998, tenaillon2004, wright2005, ross2009} which shows a smaller long-term effective population size in maize. Similarly, our observations based on singleton diversity, which reflect more recent patterns of selection, again are in agreement with maize and teosinte demography; from the viewpoint of a recent mutation, the rapid expansion of maize post-domestication means that today maize has a vastly larger effective population size than does teosinte.

\section{Demography and selection interact to shape genetic diversity} %JRI NOT DONE
Importantly, our results demonstrate that abstractions such as average long-term $N_e$ cannot capture the dynamic relationship between demography and selection. While it is certainly the case that diversity is lost during a bottleneck, it is equally true that from the viewpoint of a mutation that arises later, the bottleneck is irrelevant. Although the relatively straight-forward demographic history of maize risks oversimplifying this observation, it is applicable to any species that has undergone demographic change. For instance, human populations have experienced recent growth much like maize has, and this growth corresponds to a documented excess of rare genetic variants \cite{keinan2012, tennessen2012, coventry2010}. Our results suggest that in such scenarios, ongoing background selection may lead to a dearth of rare variants in and around genes compared to outside of them. This conflicts with an alternative possibility; that sudden expansion should lead to a comparable increase in rare variants genome-wide.

The abundance of rare variants across the genome, and the efficiency of purifying selection against them, has notable implications for genetic load. One possibility is that the excess of rare variants present in expanding populations implies higher genetic load \cite{keinan2012}. However, more efficient purifying selection in larger populations, which we show operates specifically on these rare variants, should decrease genetic load by purging rare deleterious variants. Several studies have demonstrated that despite different demographic histories, African and European human populations have a similar genetic load \cite{do2015,simons2014}, due in part to new deleterious mutations being increasingly rare as population size increases \cite{simons2014, lohmueller2014}. We propose that this observation could also result from more efficient selection against such mutations. This proposition is in agreement not only with our maize observations, but also with the results of \cite{gazave2013}, who showed via simulations that the increased efficiency of selection in a larger population can partially offset the increase in the total number of segregating deleterious variants.

Across species, our results demonstrate that the genetic impacts of major demographic events such as bottlenecks depend not only on population size reductions, but also on subsequent, potentially rapid and recent, expansion that often comes afterwards. Reduced population sizes do lead to a decreased efficiency of selection \cite[e.g]{renaut2015}, which has long-term impacts on diversity measured via $\pi$. But new mutations, which we measure with singleton diversity, are oblivious to past demography, and therefore selection for or against such sites depends primarily on recent demographic processes.
...
%\jri{some clarification here. did we use Lemmon's higher depth teosinte? If so, why does Li say we only had 7 high-depth bam files?  shouldn't there be more?}

% TB Says: Most bam files were not high-depth, but a few were. I used what was made available from Panzea group via iplant in summer 2014, and precise files that were used are linked and described in table. ANGSD GL approach makes it OK that depth varied.

\subsection{Physical and genetic maps}
Sequences were mapped to the maize B73 version 3 reference genome \cite{schnable2009} (ftp://ftp.ensemblgenomes.org/pub/plants/release-22/fasta/zea\_mays/dna/) as described by \cite{hapmap3}. \jri{update ref. 29 to hapmap 3 paper}
 All analyses made use of uniquely mapping reads with mapping quality score $\geq  30$ and bases with base quality score $\geq 20$; quality scores around indels were adjusted following Li \emph{et al.} \cite{li2011statistical}. \jri{did we really only use uniquely mapping? or just a MQ $>30$ filter?}
...
We called SNPs in ANGSD \cite{korneliussen2014} using a SNP p-value of $1e-6$ against a reference genome masked using SNPable (\url{http://lh3lh3.users.sourceforge.net/snpable.shtml}). \jri{fix url, not showing up in pdf}
We then removed heterozygous genotypes and filtered sites with a mapping quality $<30$, a base quality $<20$, or a $|\emph{log}_2(\emph{depth})|<1$. \jri{fix depth, italics is weird}
...
We called genotypes in maize, teosinte, and \emph{Tripsacum} at sites with a SNP p-value $<10^{-6}$ and when the genotype posterior probability $>0.95$. \jri{in methods, not sure if you should use emph instead of textit or something, as italics text looks weird}
